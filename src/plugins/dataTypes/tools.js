 ...
 * @returns
 */
export const toString = (variable, typeKey, tz, tabSize = 0, collection = new Set()) => {
    ...
    if (isPrimitive) { // 基础类型
        ...
        if (typeKind === 'union') {
            if (Array.isArray(typeArguments) && typeArguments.length) {
                ...
                if (typeArg) {
                    str = toString(variable, genSortedTypeKey(typeArg), tz, tabSize, collection);
                }
            }
                ...
                if (typeKind === 'generic' && typeNamespace === 'nasl.collection') {
                    if (typeName === 'List') {
                        ...
                        const tmp = variable.map((varItem) => {
                            return `${indent(tabSize + 1)}${toString(varItem, itemTypeKey, tz, tabSize + 1, collection)}`
                        });
                        ...
                        const arrStr = keys.map((key) => `${indent(tabSize + 1)}${toString(key, keyTypeKey, tz, tabSize + 1, collection)} -> ${toString(variable[key], itemTypeKey, tabSize + 1, collection)}`).join(',\n');
                        if (keys.length) {
                            str = `{\n${arrStr}\n${indent(tabSize)}}`;
                        } else {
                            str = '{}';
                    ...
                    if (Array.isArray(properties) && properties.length) {
                        code += properties.map((property) => {
                            ...
                            const propValStr = toString(propVal, propTypeKey, tz, tabSize + 1, collection);
        ...
        if (Object.prototype.toString.call(variable) === '[object Object]') {
            if (collection.has(variable)) {
                ...
                for (const key in variable) {
                    ...
                    const propValStr = toString(propVal, undefined, tz, tabSize + 1, collection);
